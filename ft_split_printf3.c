...
/*   Updated: 2017/12/14 22:17:52 by arobion          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_prout(char **tab, int **ret, char *s, int *size_word)
{
	int		i;
	int		j;
...
static int	ft_merde(char *s, char ***tab, int ***ret, int **size_word)
{
	int nb_word;
...
int			**ft_strsplit_printf(char *s)
{
	...
	i = 0;
	if (!(nb_word = ft_merde(s, &tab, &ret, &size_word)))
		return (NULL);
	while (i < nb_word)
	{
