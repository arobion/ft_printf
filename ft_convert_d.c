...
/*   Updated: 2017/12/15 18:57:58 by arobion          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

char					*ft_flags_d(long long var, t_param param)
{
	int		i;
	char	*str;
	...
	if (var == 0 && param.precision == 0 && param.width == 0)
		return (str);
	if (((ft_is_in(param.flags, '+') || ft_is_in(param.flags, ' ')) && var >= 0) && (param.precision >= (int)param.width || ((int)param.width <= ft_count_var(var) && param.precision == -1)))
		i++;
